...
designFile = [x for x in proj.files if "rest_acompcor_bandpass.dsn" in x.name][0]
...
for i, x in enumerate(sessions):
    ...
    if  fmriprep:
        ...
        jobid = xcp.run(analysis_label="XCP_SDK_NO_TASK_REGRESS{}".format(datetime.datetime.now()), destination=ses, inputs=myinput, config=myconfig)
