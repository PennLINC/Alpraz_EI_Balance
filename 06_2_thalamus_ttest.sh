...
echo "subject FD sex age distance NRCount" > $cov_file
...
sed 1d $sublist |while IFS=, read subid sesid age sex FD distance NRCount other; do
	...
	echo "$sub_label ${dataDir}/thalamus_conn_fisherz.nii.gz" >> $scan_file
	echo "$sub_label $FD $sex $age $distance $NRCount" >> $cov_file
...
3dttest++ -prefix "/cbica/projects/alpraz_EI/output/PNC_ttests/thalamus_conn_distance_clustsim.nii.gz" -overwrite -mask $mask -setA thalamusFC $scans -covariates $cov_file -Clustsim 20 -overwrite
